...
class OutputStreamAdapter(
    private val writer: (ByteArray,Int,Int) -> Unit
): OutputStream() {
    override fun write(byte: Int) {
        writer(byteArrayOf(byte.toByte()), 0, 1)
    }

    override fun write(bytes: ByteArray) {
        writer(bytes, 0, bytes.size)
    }

    override fun write(bytes: ByteArray, start: Int, length: Int) {
        writer(bytes, start, length)
    }
}
...
class MapCollectionAdapter(private val src: Collection<Any?>): AbstractMutableMap<String,Any?>() {
    override val entries
        get() = mutableSetOf<MutableMap.MutableEntry<String,Any?>>(KeyValueReference("size", src.size, true))
    override val size
        get() = src.size
    override val keys
...
class MultiMap(vararg originalMaps: MutableMap<Any?,Any?>) : MutableMap<Any?, Any?> {
    private val mapsList = mutableListOf<MutableMap<Any?,Any?>>()
    init {
    ...
    val maps: List<Map<Any?,Any?>> get() = mapsList
    fun add(map: MutableMap<Any?, Any?>): MultiMap {
        mapsList.add(map)
        return this
    }

    override val entries: MutableSet<MutableMap.MutableEntry<Any?, Any?>> get() {
        val set = mutableSetOf<MutableMap.MutableEntry<Any?, Any?>>()
        for (map in mapsList)
            set.addAll(map.entries)
        return set
    }

    override val keys: MutableSet<Any?> get() {
